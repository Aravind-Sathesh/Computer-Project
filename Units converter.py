def convertor(flag):
    ...
    def compute(unit_dict):
        clear()
        print(
            'Choose the unit to convert from: [1-', len(unit_dict), ']', sep='')
        ch3 = choose(unit_dict)
        if 0 < ch3 <= len(unit_dict):
            clear()
            print(
                'Choose the unit to convert to: [1-', len(unit_dict), ']', sep='')
    ...
    main_dict = {1: 'Length', 2: 'Mass', 3: 'Temperature',
                 4: 'Volume', 5: 'Storage', 6: 'Energy'}

    # Following dictionaries store the name of the unit as the key and the product for conversion to base unit as the value:
    length_dict = {'Angstrom': 10**-10, 'Nanometre': 10**-9, 'Micrometre': 10**-6, 'Millimetre': 10**-3, 'Centimetre': 10**-2, 'Decimetre': 10**-1, 'Metre': 1,
                   'Kilometre': 10**3, 'Megametre': 10**6, 'Foot': 0.3048, 'Yard': 0.9144, 'Inch': 0.0254, 'Parsec': 30857*10**12, 'Light-year': 946*10**13, 'Astronomical Unit': 1496*10**8}
    mass_dict = {'Microgram': 10**-6, 'Milligram': 10**-3, 'Gram': 1, 'Kilogram': 10**3, 'Metric ton': 10**6,
                 'Tonne': 1.016*10**6, 'Pound': 453.59, 'Ounce': 28.349, 'Carat': 0.2, 'Atomic Mass Unit': 1.66*10**-24}
    temp_list = ['Celsius', 'Fahrenheit', 'Kelvin']
    volume_dict = {'Cubic metre': 1, 'Litre': 10**-3, 'Cubic centrimetre': 10**6, 'Cubic foot': 28.316 *
                   10**-3, 'Cubic inch': 16.387*10**-6, 'Pint': 0.568261*10**-3, 'Gallon': 4.546*10**-3, 'Barrel': 0.15989}
    storage_dict = {'Bit': 0.125, 'Nibble': 0.5, 'Byte': 1, 'Kilobyte': 1024, 'Megabyte': 1024**2, 'Gigabyte': 1024 **
                    3, 'Terabyte': 1024**4, 'Petabyte': 1024**5, 'Exabyte': 1024**6, 'Zettabyte': 1024**7, 'Yottabyte': 1024**8}
    energy_dict = {'Erg': 10**-7, 'Joule': 1, 'Kilojoule': 10**3, 'Megajoule': 10**6, 'Gigajoule': 10**9, 'Calorie': 4.184, 'Kilocalorie': 4184, 'Electron volt': 1.6022*10**-19,
                   'Watt-hour': 3.6*10**3, 'Kilowatt-hour': 3.6*10**6, 'Foot-pound force': 1.3558, 'British Thermal Unit (BTU)': 1055.0559, 'Barrel-of-oil equivalent': 6117863199.99928, 'Therm': 105505590}
    ...
    if ch2 == 1:
        ...
        if 0 < ch3 <= (len(temp_list)):  # To check if the value is within list limits
            ...
            if 0 < ch4 <= (len(temp_list)):  # To check if the value is within list limits
                ...
                print('To:', to_unit)
                print(input_val, from_unit.lower(), 'is',
                      output_val, to_unit.lower(), end='.\n')
                print('-----------------------------------------------------')
            else:
                else_block()
        else:
